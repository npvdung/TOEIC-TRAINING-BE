...
async function generateRandomExam(questionCategoryRequireList) {
    ...
    for (let questionCategoryRequire of questionCategoryRequireList) {
        if (questionCategoryRequire.type == 1) {
            query += " UNION (SELECT * " +
                " FROM questions " +
                " WHERE questionCategory = :categoryId" + i + " AND questionLevel = :questionLevel" + i +
                " ORDER BY RAND() " +
                " LIMIT :quantity" + i + ") ";
            ...
            const [readingListQuery, metadata] = await db.sequelize.query("SELECT * " +
                " FROM readings " +
            ...
            let readingListQueryIdList = readingListQuery.map(reading => reading.id);
            query += " UNION (SELECT * " +
                " FROM questions " +
                " WHERE readingId IN (:readingListQueryIdList" + i + ") " +
                " ORDER BY RAND()) ";
            replacements["readingListQueryIdList" + i] = readingListQueryIdList;
            i++;
        }
    }
    query = query.slice(" UNION".length);
    query = "SELECT * FROM (" + query + ") as question ORDER BY questionCategory ASC, questionLevel ASC ";
...
async function getRandomQuestionByCategoryId(questionCategoryRequire) {
    const [result, metadata] = await db.sequelize.query("SELECT * " +
        "FROM questions " +
